...
def max_duffel_bag_value(cake_tuples, weight_capacity):
    ...
    # create an array with ROI value (tuple[1]/tupel[0]
    cake_roi = []
    for weight, value in cake_tuples:
        print(cake_tuples[1], cake_tuples[2])
        cake_roi.append(weight * value)
    # base on highest ROI choose that cake up until adding 1 more would put us over capacity
    # then perform the same thing with the next highest ROI cake
    # return total value of cakes
...
class Test(unittest.TestCase):
    ...
    def test_lots_of_cakes(self):
        actual = max_duffel_bag_value(
            [(2, 3), (3, 6), (5, 1), (6, 1), (7, 1), (8, 1)], 7)
        expected = 12
        self.assertEqual(actual, expected)
